            ...
            <div class="card-body order-table">
                <table id="table1">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="avatar avatar-lg me-3">
                                    <a href="{{asset('uploads/CakeBanner/'.$banner->image)}}">
                                        <img src="{{asset('uploads/CakeBanner/'.$banner->image)}}" alt="{{$banner->image}}" srcset="">
                                    </a>
                                </div>
                            </td>
                            <td>{{$banner->status}}</td>
                            <td>
                                <a href="{{route('admin.cakebanner.edit',$banner->id)}}" class="btn icon btn-sm btn-primary"><i class="far fa-edit"></i></a>
                                <a href="#" class="btn icon btn-sm btn-info"><i class="far fa-eye"></i></a>
                                <form action="{{route('admin.cakebanner.destroy',$banner->id)}}" method="POST" onsubmit="return confirm('Are you sure to delete?')">
                                    @method('DELETE')
                                    @csrf
                        ...
                        @empty
                        <p>There are currently no banners for cake...</p>
                        @endforelse
                    </tbody>
                </table>
            </div>
